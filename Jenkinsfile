    ...
    stages {
        ...
        stage("Deploy to Dev")
        {
            steps{
                ...
                script{
                 sh "sshpass -p ${PASSWORD} ssh -o StrictHostKeyChecking=no ${USERNAME}@${docker_server_ip} docker run -P --env ENVIRONMENT=dev --name ${env.APPLICATION_NAME}-${env.AREA}-${GIT_COMMIT} ${env.DOCKER_HUB}/${env.APPLICATION_NAME}:${GIT_COMMIT}"
                }
                }
            }
        }
        ...
        //         script{
        //          sh "sshpass -p ${PASSWORD} ssh -o StrictHostKeyChecking=no ${USERNAME}@${docker_server_ip} docker run -P ${env}=test --name ${env.APPLICATION_NAME}-${env.AREA}-${GIT_COMMIT} ${env.DOCKER_HUB}/${env.APPLICATION_NAME}:${GIT_COMMIT}"
        //         }
        //         }
        //     }
        // }
